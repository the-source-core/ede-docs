...
| [Dataset & Metric Engine](foundation-dataset.md) | `foundation.dataset` (Phase 1 ✅ Delivered 2026-05-13 — substrate + Blueprint admin form + HTTP + RBAC + demo; Phase 2 ✅ Delivered 2026-05-13 — plan engine + formula engine + DAG cycle detection + shared safe AST evaluator + per-run cache + 3 new demo metrics; Phase 3 ✅ Delivered 2026-07-07 — chip · period · SSE/WS streaming · webhook · csv/xlsx/parquet export · Redis-backed cross-request query cache · live `dataset.stream` client action) | [roadmap/foundation/dataset/](../roadmap/foundation/dataset/README.md) |
...
| [Notifications Engine](foundation-notifications.md) | `foundation.notifications` (Phase 1 ✅ · **Phase 2 ✅ Delivered 2026-07-07** — per-user preferences + quiet hours + dedup + digest + correlation rollup/recall + delivery log + admin analytics dashboard) | [roadmap/foundation/notifications/](../roadmap/foundation/notifications/README.md) |
...
| [Storage (Document Store + Pluggable Backends)](foundation-storage.md) | `foundation.storage` (✅ Delivered — baseline, pre-roadmap) — `storage.folder` + `storage.document` + `storage.document_version` + `StorageRouter` (local FS default; cloud via `foundation.connectors` — Google Drive shipped) + HTTP API + RBAC seed + Documents app. **Enh 01 Frontend Drive:** P1 MVP + P2 three-pane UX ✅ Delivered (browser-verified 2026-06-10); P3 core delivered (residuals pending); P4 Documents section 🔴 | [roadmap/foundation/storage/](../roadmap/foundation/storage/README.md) |
...
| [Declarative Constraints](platform-15-declarative-constraints.md) 🔴 | Two parallel constraint-declaration surfaces (replaces the rejected single `sql=True` mode-flag). **Phase 1 — DB-driven:** new `constraints.*` kernel namespace (imported like `fields`) — `constraints.Check("min_qty <= max_qty", message=...)` + `constraints.Unique("code", "org_id", ...)` class-body descriptors; attribute name = DB constraint identity (rename-safe migration diffing + `IntegrityError`→friendly-`message` translation); scanned by the metadata builder and emitted by `ede migrate generate` (named, ≤63-char, SQLite batch-rebuild). **Phase 2 — code-driven:** `@api.constrains(*fields)` decorator expanding into `pre.create`+`pre.update` hooks with the record pre-merged + trigger-field gating — removes the two-hook + manual-merge boilerplate. Spans `ede.core.kernel` + `ede.core.api` + `ede.core.registry` + sqlalchemy metadata builder + command bus. | [roadmap/platform/15-declarative-constraints.md](../roadmap/platform/15-declarative-constraints.md) |
