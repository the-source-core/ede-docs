...
*Last sync: 2026-05-18 (Enhancement 02 ✅ Delivered — lazy + secret-keyed `default_secrets` cache so `ede.conf`-loaded Fernet key actually reaches the encryption hook · `get_extra_params` moved into the provider builder so the api_key param is readable via `RecordSet`. Phases 1 / 2 / 4 ✅ Delivered (2026-05-15 / 2026-05-17) · Enhancement 01 ✅ Delivered (2026-05-15) · Phase 3 🔴 ON HOLD pending `foundation.jobs` P1). To refresh, invoke the `syncing-roadmap-to-docs` skill.*
