...
| Frontend Drive (DMS UI) on `src/frontend` — `foundation.storage` Phase 0 backend ships complete, but the new workspace client has no surface yet (the Documents menu routes to a stub). Legacy `src/frontend/src/workspace/views/documents/` carries the working UI. Ships in two coordinated surfaces sharing one plugin: menu-attached `open_storage_drive` (full-page Drive at `/wc/<org>/storage-drive`) + button-attached `open_record_documents` (Radix Dialog scoped to a record's documents) — both built atop foundation.presentation Enhancement 08's client-action lifecycle. Pulls in PDF.js (~600KB) lazy-loaded for inline preview; native browser elements for image / text MIMEs. Backend adds preview-signed-URL endpoint + cross-folder search + move/copy commands (~3 commands, ~3 routes, per-connector `sign_download_url` extension method). Tests planned: 24 pytest + 22 vitest = 46 cases. | 🔴 | [Storage Enhancement 01](../roadmap/foundation/storage/enhancements/01-frontend-drive-client-action.md) |
<!-- /SYNC-BLOCK -->
