...
| In-app recorder absent in `src/frontend` — currently lives in legacy `src/frontend/src/qa-recorder/*` behind the `qa_automation_enabled` feature flag. Legacy-frontend deprecation A10/D3 resolved as **Port behind flag** (not drop) — recorder ships as a `frontend` client-action plugin via Enhancement 08's SDK, gated on the same flag. Not a cutover blocker; ships on its own cadence after `src/frontend` is deleted. | 🔴 | [enhancements/04-recorder-port-to-frontend2.md](../roadmap/foundation/qa-automation/enhancements/04-recorder-port-to-frontend2.md) |
<!-- /SYNC-BLOCK -->
