...
| **Enhancement 01 — Readable Cron Builder `SchedulerCron`** (✅ 2026-05-31) | uses existing `ir.job.cron` string column; no schema change | `src/ede/foundation/jobs/services/cron.py` (`SchedulerCron` class — `run_every(minutes=\|hours=\|days=)`, anchored `minutely`/`hourly`/`daily`/`weekly`/`monthly`, general `build(minute=, hour=, day=, month=, weekday=)` with int/list/str fields + weekday names & ranges; each method validates via `validate_cron_expression` before returning) · `src/ede/foundation/jobs/__init__.py` (re-export) · `src/tests/foundation/jobs/test_cron_builder.py` (24 cases). Pure call-site sugar over `cron=`; raw strings still work. | [enhancements/01-cron-builder.md](../roadmap/foundation/jobs/enhancements/01-cron-builder.md) |
<!-- /SYNC-BLOCK -->
...
*Last sync: 2026-05-31 (Enhancement 01 ✅ Delivered — `SchedulerCron` readable cron builder added to Built Capabilities: `run_every` / anchored helpers / general `build`, validated at import, pure sugar over `cron=`; 24 pytest cases; no schema/settings change. Prior sync: 2026-05-28 (Phase 3 ✅ Delivered — approval SLA escalation adopted onto foundation.jobs as the `approval.sla_tick` scheduled job; WS-J21 notifications webhook deferred)). To refresh, invoke the `syncing-roadmap-to-docs` skill.*
