...
**Status:** 🟡 In Progress — Phase 1 ✅ Delivered 2026-05-13 (engine + HTTP API + React chatter UI + `<activity>` DSL + `Chatterable` mixin + `res.partner` proof-of-life); **Phase 2 ✅ Delivered 2026-06-24** (notifications fan-out per follower flag, outbound email bridge, auto-follow, `@mention` parsing, identity centralization via the delegation kernel, ~15 consumer models adopted, and partner-centric recipient resolution that closes external-follower notification delivery); Phase 3 🔴 Not Started.
...
| Phase 2 | Notifications + Email bridges, cross-domain rollout, mentions, auto-follow | ✅ Delivered 2026-06-24 | [phase-2-implementation.md](../roadmap/foundation/communication/phase-2-implementation.md) |
...
| **Enhancement 03 — Record Strip + Unified Timeline (frontend2 chatter)** (2026-06-11) | reuses Phase 1 models; `("tracking","Field Change")` added to `communication.message.message_type` (migration-free); kernel `Field(tracking=True)` flag | **Gating:** [src/ede/core/services/presentation/activity_gates.py](../src/ede/core/services/presentation/activity_gates.py) (`<activity>` × `Chatterable`/`Attachable` boot loud-fail + serve-time filter in `strip_render_plan`) · **Tracking:** [hooks/field_tracking.py](../src/ede/foundation/communication/hooks/field_tracking.py) (create/update lifecycle logs + per-field `tracking=True` diffs with Reference display-name resolution) · **Bridges:** [services/timeline_bridges.py](../src/ede/foundation/communication/services/timeline_bridges.py) (workflow stages, approval decisions + escalations → timeline, event-name coupling only) · [services/mention_alerts.py](../src/ede/foundation/communication/services/mention_alerts.py) (mention → dispatcher for internal users, `mail.outbox` for externals) · **Workflow viewer:** `GET /api/workflow/graph` in [workflow_routes.py](../src/ede/foundation/workflow/api/workflow_routes.py) · **Frontend:** `RecordStrip` / `RecordDocumentsDrawer` / `FollowersPopover` / `RecordTimeline` / `RecordComposer` (slash palette + grouped @mention with inline chips + `@followers`) / `WorkflowDrawer` under [src/frontend/src/managers/](../src/frontend/src/managers/) | [enhancements/03-form-chatter-record-strip-timeline-ux.md](../roadmap/foundation/communication/enhancements/03-form-chatter-record-strip-timeline-ux.md) |
| **Phase 2 — Notifications + Email bridges, mentions, auto-follow, identity centralization, partner-recipient resolver** (2026-06-24) | reuses Phase 1 models; `communication.follower.source` Enum (`manual`/`auto_owner`/`mention`); identity via `res.user.partner_id` ↔ `res.partner.user_ids` (delegation kernel) | **WS-1 notifications fan-out:** [services/notifications_bridge.py](../src/ede/foundation/communication/services/notifications_bridge.py) (`@api.on_event("communication.message.posted")`, per-follower flag, `note` suppressed, external follower → `{"partner_id": …}` spec) · **WS-2 email:** [services/email_bridge.py](../src/ede/foundation/communication/services/email_bridge.py) (`communication.post_email` → `mail.outbox` + `mail_outbox_id`) + `POST /api/chatter/{model}/{id}/email` in [api/chatter_controller.py](../src/ede/foundation/communication/api/chatter_controller.py) · **WS-3 auto-follow:** [services/auto_follow.py](../src/ede/foundation/communication/services/auto_follow.py) (`@api.on_event("ede.record.created")`, `__ede_auto_follow_fields__`, `source="auto_owner"`) · **WS-4 mentions:** [services/mention_parser.py](../src/ede/foundation/communication/services/mention_parser.py) + [services/mention_alerts.py](../src/ede/foundation/communication/services/mention_alerts.py) · **Partner-recipient resolver:** [notifications/services/recipient_resolver.py](../src/ede/foundation/notifications/services/recipient_resolver.py) (partner-centric — resolves a `recipient_spec` of `partner_id`/`partner_ids`/`role` to `res.partner` UUIDs; the dispatcher persists one `ir.notification` per partner and resolves `res.partner.user_ids` at the delivery boundary) · **WS-5 adoption:** ~15 consumer models across base / pricing / shipments / sales_crm / consolidation · **Seed:** [data/notification_templates.xml](../src/ede/foundation/communication/data/notification_templates.xml) · communication + notifications suites green | [phase-2-implementation.md](../roadmap/foundation/communication/phase-2-implementation.md) |
<!-- /SYNC-BLOCK -->
...
|---|---|---|
| ~~Form chatter absent in frontend2~~ — **closed by Enhancement 03** (record strip + unified timeline, ✅ 2026-06-11); Enhancement 01's right-rail surface was superseded by the frozen strip+timeline UX. | ✅ closed | [Communication Enhancement 03](../roadmap/foundation/communication/enhancements/03-form-chatter-record-strip-timeline-ux.md) |
| Cross-record Activity Notification Pane absent — top-bar bell is placeholder in both legacy and frontend2; needs new `chatter.activities_for_me` backend command + `GET /api/chatter/activities-for-me` route + frontend pane reusing the GeneralNotificationPane chrome. Closes the "where are my scheduled activities across all records" UX gap. | 🔴 | [Communication Enhancement 02](../roadmap/foundation/communication/enhancements/02-activity-notification-pane-frontend2.md) |
| ~~Phase 2 WS-0 (Identity centralization): `res.user` has no `partner_id` Reference~~ — **closed** (✅ 2026-06-24): `res.user.partner_id` ↔ `res.partner.user_ids` shipped via the Enh 08 delegation kernel (auto-creates the partner shadow + syncs name/email). | ✅ closed | [phase-2-implementation.md §WS-0](../roadmap/foundation/communication/phase-2-implementation.md) |
| ~~Auto-follow service not wired into `post.{model}.create`~~ — **closed** (✅ 2026-06-24): [auto_follow.py](../src/ede/foundation/communication/services/auto_follow.py) subscribes `ede.record.created` and walks `__ede_auto_follow_fields__` (`source="auto_owner"`). | ✅ closed | [phase-2-implementation.md §WS-3](../roadmap/foundation/communication/phase-2-implementation.md) |
| ~~Mention parser not wired for `@token` resolution~~ — **closed** (✅ 2026-06-24): [mention_parser.py](../src/ede/foundation/communication/services/mention_parser.py) resolves `@tokens` against `res.partner` name/email + `mention_alerts.py` delivers (internal → dispatcher, external → `mail.outbox`). | ✅ closed | [phase-2-implementation.md §WS-4](../roadmap/foundation/communication/phase-2-implementation.md) |
| ~~Notifications fan-out bypass-able from non-`Command` paths~~ — **closed** (✅ 2026-06-24): [notifications_bridge.py](../src/ede/foundation/communication/services/notifications_bridge.py) is a `communication.message.posted` event subscriber; external followers now dispatch via the partner-recipient resolver (pure external contacts, no account, are reached over email by the email / mention path). | ✅ closed | [phase-2-implementation.md §WS-1](../roadmap/foundation/communication/phase-2-implementation.md) |
...
*Last sync: 2026-06-24 (**Phase 2 ✅ Delivered** — notifications fan-out per follower flag · outbound email bridge (`communication.post_email` → `mail.outbox` + `POST /api/chatter/{model}/{id}/email`) · auto-follow (`source="auto_owner"`) · `@mention` parsing + alerts · identity centralization (`res.user.partner_id` ↔ `res.partner.user_ids` via delegation kernel) · WS-5 over-delivered (~15 consumer models) · **partner-centric recipient resolution** on the notifications `RecipientResolver` (resolves a `recipient_spec` to `res.partner` UUIDs; external followers are addressed directly as `{"partner_id": …}` and the dispatcher resolves each partner's `user_ids` at the delivery boundary) closes external-follower notification delivery; communication + notifications suites green. Previous sync: 2026-06-11 (**Enhancement 03 — Record Strip + Unified Timeline ✅ Delivered**, slices 1–7: `<activity>`×mixin availability gating (boot loud-fail + gated render plan) · record strip chips in the form toolbar · in-place documents drawer (upload/delete/preview/thumbnails/resize) · followers popover with `communication.follower` flags (follow defaults: Messages+Emails only) · unified day-grouped timeline with persisted field-change tracking (kernel `Field(tracking=True)`, lifecycle create/update logs, Reference display-name resolution) · workflow/approval/escalation timeline bridges · slash composer with grouped @mention (inline chips, pinned `@followers`, name_search_fields filtering) · mention alerts (internal → NotificationDispatcher `chatter.mention`; external → `mail.outbox`) · workflow flow-viewer drawer over the new `GET /api/workflow/graph`. Enhancement 01's right-rail UX superseded; Enhancement 02 (activity bell) remains 🔴. Previous sync: 2026-06-06 — **Enhancement 01 Form Chatter** at [`roadmap/foundation/communication/enhancements/01-form-chatter-frontend2.md`](../roadmap/foundation/communication/enhancements/01-form-chatter-frontend2.md) and **Enhancement 02 Activity Notification Pane** at [`roadmap/foundation/communication/enhancements/02-activity-notification-pane-frontend2.md`](../roadmap/foundation/communication/enhancements/02-activity-notification-pane-frontend2.md). Both authored as part of the [legacy-frontend deprecation track](../roadmap/foundation/presentation/frontend-ui-revamp/legacy-deprecation.md) — the deprecation cutover deletes `src/frontend` without porting these UIs; they ship fresh in `frontend` afterward, not as cutover blockers. Enhancement 01 covers the right-rail FormAside (Activity / Messages / Followers / Files tabs) backed by the existing `/api/chatter/*` API — no backend changes. Enhancement 02 ships a new `chatter.activities_for_me` command + route + top-bar pane. Previous sync: 2026-05-13 — Phase 1 ✅ Delivered. To refresh, invoke the `syncing-roadmap-to-docs` skill.)*
