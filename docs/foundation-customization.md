...
**Status:** ✅ Delivered (module complete) — Phase 1 (registry mirror + Properties + UI), Phase 2 (JSONB-path search), Phase 4 (anchored property fields · DB-backed views · AI-assisted customization) all ✅. Phase 3 (runtime-DDL manual fields) **descoped 2026-06-12 — Won't Do**.
...
| Phase 2 | JSONB-Path Search Domains | ✅ Delivered (2026-06-12) — `properties.<key>` domain compiler (value-driven casts · `is_set`/`is_not_set` · m2m containment) · PG GIN index on `(properties::jsonb)` · auto property search fields (text-typed); typed-input widgets deferred | [roadmap](../roadmap/foundation/customization/phase-2/README.md) |
| Phase 4 | Anchored Property Fields · DB-Backed Views · AI-Assisted Customization | ✅ Delivered (2026-06-12) — A (substrate + list/kanban cell extraction) · B (DB-backed views, walkthrough confirmed) · C (AI-assisted: `view_parent_id` enabler · proposer tool + pack · safety/RBAC tests · frontend confirm flow; chat-driven browser e2e deferred — AI-provider stub) | [roadmap](../roadmap/foundation/customization/phase-4/README.md) |
| ~~Phase 3~~ | ~~Manual Fields via Runtime DDL~~ | 🟢 Descoped (2026-06-12) — Won't Do | _Runtime column creation unsupported; all schema via migrations; Properties (Phase 1) + JSONB search (Phase 2) cover small-field needs. Revisit only for marketplace / custom-extension apps._ |
<!-- /SYNC-BLOCK -->
...
| Anchored `<property name="bag:key"/>` element + property-bag registry (Phase 4 Slice A) | _(no models — DSL element + registry primitive + synthesized FieldDefs)_ | [registry.py](../src/ede/core/registry.py) (`register_property_bag`), [parser.py](../src/ede/core/services/presentation/dsl/parser.py) (`<property>` branch), [presentations.py](../src/ede/foundation/presentation/models/presentations.py) (`_synthesize_property_field_defs`), [FormSheet.tsx](../src/frontend/src/managers/FormSheet.tsx) (`FormPropertyRow`), [cell.ts](../src/frontend/src/managers/cell.ts) (`readFieldValue`/`writeFieldValue` — shared list/kanban cell value-extraction through `record[bag][key]`) | [Phase 4 / 01](../roadmap/foundation/customization/phase-4/01-property-element-and-bag-registry.md) |
| AI-assisted view customization (Phase 4 Slice C) — ask the assistant to add a field; confirm card writes it | _(uses `ir.model.property.*` + `ir.application.view`)_ | [assistant_tools.py](../src/ede/foundation/assistant/tools/assistant_tools.py) (`propose_custom_field`), [seed_skill_packs.xml](../src/ede/foundation/assistant/data/seed_skill_packs.xml) (`assistant.customization` pack), [view_composer.py](../src/ede/core/services/presentation/view_composer.py) (`primary_view_uuid` → `view_parent_id`), [CustomFieldConfirmCard.tsx](../src/frontend/src/managers/assistant/CustomFieldConfirmCard.tsx), [AssistantPanel.tsx](../src/frontend/src/managers/assistant/AssistantPanel.tsx) (`handleCustomFieldConfirm`) | [Phase 4 / 03](../roadmap/foundation/customization/phase-4/03-ai-driven-view-customization.md) |
| JSONB-path search (Phase 2) — `("properties.<key>", op, value)` domains + auto property search fields | _(no models — domain-compiler + metadata-builder + presentation)_ | [generic_repo.py](../src/ede/core/adapters/persistence/sqlalchemy/generic_repo.py) (`_compile_property_leaf`), [domain_filter.py](../src/ede/core/services/persistence/domain_filter.py) (`is_set`/`is_not_set`), [metadata_builder.py](../src/ede/core/adapters/persistence/sqlalchemy/metadata_builder.py) (`_attach_property_gin_index`), [presentations.py](../src/ede/foundation/presentation/models/presentations.py) (`_generate_property_searchfields`) | [Phase 2](../roadmap/foundation/customization/phase-2/README.md) |
<!-- /SYNC-BLOCK -->
...
*Last sync: 2026-06-12 (module now ✅ complete — Phase 3 runtime-DDL manual fields **descoped — Won't Do**; runtime column creation unsupported, all schema via migrations. With Phases 1/2/4 ✅ and Phase 3 dropped, customization is fully delivered. Phase-count effect: Foundation 80→79, Total 114→113. Prior same day: Phase 2 ✅, Phase 4 ✅). To refresh, invoke the `syncing-roadmap-to-docs` skill.*
