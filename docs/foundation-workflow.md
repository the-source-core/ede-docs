...
| **Team-Role Integration** — 3 new safe-AST guard helpers (`team_has_role`, `team_role_user`, `team_role_users`) extending the guard expression vocabulary + 2 new transition action kinds (`notify_team_role` dispatches notification to team's role-holder; `assign_team_role` auto-stamps team-role-holder into a record field). Both consume the shared `TeamRoleService` from `foundation.base` Enh 06 (hard prereq). New `team_role_lookup_failed` event-log type with warn/error/silent severity per `workflow.team_role.lookup_failure_severity` ir.config key. Sibling of `foundation.approval` Enh 01. First consumer: `logistics.sales-crm` Enh 09 (auto-assign team's ACCOUNT_MANAGER on lead→opportunity conversion; notify team's REVIEWER on quote submit). | 🔴 Not Started (drafted 2026-05-25) | [enhancements/01-team-role-integration.md](../roadmap/foundation/workflow/enhancements/01-team-role-integration.md) |
<!-- /SYNC-BLOCK -->
