...
|---|---|---|
| Form chatter (Activity / Messages / Followers / Files tabs) absent in `src/frontend` — backend `/api/chatter/*` ships complete; the new workspace client has no right-rail chatter surface. Records lose discussion + activity + attachment + follower UX after legacy cutover until this lands. Ships fresh per Enhancement 01 (Radix Tabs, react-markdown composer, EventBus-driven live badges) — not a port of legacy `views/chatter/*`. | 🔴 | [Communication Enhancement 01](../roadmap/foundation/communication/enhancements/01-form-chatter-frontend2.md) |
