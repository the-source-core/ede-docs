...
**Status:** 🟡 In Progress — Phases 1 / 2 / 4 ✅ Delivered (P1 on 2026-05-15, P2 + P4 on 2026-05-17); Phase 3 (Embeddings + Semantic) 🔴 ON HOLD pending `foundation.jobs` Phase 1
...
| **Plaintext-key mode** — when `AI_PROVIDER_KEY_ENCRYPTION_SECRET` is unset, `api_key_encrypted` + secret params are stored as plaintext (loud boot WARNING). Acceptable for dev/personal; production tenants MUST set the secret before storing real keys. | 🟡 | — |
...
*Last sync: 2026-05-17 (Phases 1 / 2 / 4 ✅ Delivered + Fernet encryption + per-provider param O2M + DB-driven provider builder · Enhancement 01 ✅ Delivered — standalone AI app + DSL views · Phase 3 🔴 ON HOLD pending `foundation.jobs` P1). To refresh, invoke the `syncing-roadmap-to-docs` skill.*
