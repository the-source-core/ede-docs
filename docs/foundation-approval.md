...
| Approval Builder (admin config UI) | n/a — configures `ir.approval.policy.set` / `ir.approval.rule` / `ir.approval.flow.template` / `ir.approval.flow.step.def` / `ir.approval.escalation.policy` | `approval.builder` client action (menu **Approvals → Configurations → Builder**, first item) — [`BuilderPage.tsx`](../src/ede/foundation/approval/frontend/components/BuilderPage.tsx) (home tabs: flows grouped by policy domain with active toggle + guarded delete · escalation-policy library CRUD with used-by guard · team-role defaults over the approval settings module), [`BuilderWizard.tsx`](../src/ede/foundation/approval/frontend/components/BuilderWizard.tsx) (5-step wizard + live-preview rail; chain editor with USER/ROLE/TEAM_ROLE pickers, per-step SLA + breach policy + ALL/ANY join, team-field binding), [`conditionTree.ts`](../src/ede/foundation/approval/frontend/components/conditionTree.ts) (nested all/any condition tree: parenthesized `and`/`or` compose + round-trip parser, advanced raw-expression fallback), [`builderData.ts`](../src/ede/foundation/approval/frontend/components/builderData.ts) (rule+template+steps bundle persistence via generic RecordService — zero backend changes); 19 vitest cases | [Approval Enhancement 04](../roadmap/foundation/approval/enhancements/04-approval-builder-frontend2.md) (frozen [mockup](../roadmap/foundation/approval/mockups/approval-builder-frozen.html)) |
